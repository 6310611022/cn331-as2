...
def quota_request(request):
    if not request.user.is_authenticated:
        ...
        other_course = Course.objects.exclude(id__in=course_apply.values_list('id', flat=True)).filter(status=True)
...
def cancel(request, course):
    if not request.user.is_authenticated:
        ...
        cancel_course = Course.objects.filter(course=subject_course, semester=course[1], year=course[2]).first()
...
def request(request, course):
    if not request.user.is_authenticated:
        ...
        student_info = Student.objects.get(student=student)
        subject_course = Subject.objects.get(subject_id=course[0])
        request_course = Course.objects.filter(course=subject_course, semester=course[1], year=course[2]).first()
